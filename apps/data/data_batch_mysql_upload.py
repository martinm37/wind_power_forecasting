...
if len(already_present_observations) == 0:
    ...
    already_present_observations_df = pd.DataFrame(
        data=already_present_observations, columns=["datetime"]
    )
    ...
    difference_set = (
        new_set - present_set
    )  # observation in the new data, that are not in the DB

    difference_df = pd.DataFrame(data=difference_set, columns=["datetime"])
    difference_df["datetime"] = pd.to_datetime(
        difference_df["datetime"], utc=True
    )  # converting to time
    ...
    df_to_insert = pd.merge(
        left=data_df, right=difference_df, on="datetime", how="inner"
    )
