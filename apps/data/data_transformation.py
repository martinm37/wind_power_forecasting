...
data_selection["Datetime"] = pd.to_datetime(
    data_selection["Datetime"], utc=True
)  # converting to time
data_selection["Datetime"] = data_selection["Datetime"].dt.tz_convert(
    None
...
rescaled_power_vec_lower_bounded = np.maximum(
    rescaled_power_vec, np.zeros(len(power_vec))
)  # .reshape(-1,1)
rescaled_power_vec_bounded = np.minimum(
    rescaled_power_vec_lower_bounded, np.ones(len(power_vec)) * 100
)
