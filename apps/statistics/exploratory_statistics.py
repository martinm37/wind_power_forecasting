...
data_matrix = data[
    ["Measured & Upscaled", "Monitored capacity", "Rescaled Power"]
].to_numpy()
...
autocorrelation_function_vec = acf_comp(
    y_vec=rescaled_power_vec, total_lag_k=4 * 24 * 1
)
...
autocorrelation_function_vec = acf_comp(
    y_vec=rescaled_power_vec, total_lag_k=4 * 24 * 30 * 12 * 5
)
time_length = len(rescaled_power_vec)
