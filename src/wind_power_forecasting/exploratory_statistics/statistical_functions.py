...
def acf_comp(y_vec, total_lag_k):
    ...
    y_variance = (
        y_vec_demeaned.T @ y_vec_demeaned
    )  # WITHOUT dividing - as it cancels out later on
...
if __name__ == "__main__":
    ...
    data_selection["Measured & Upscaled"] = data_selection[
        "Measured & Upscaled"
    ...
    data_select = data_selection[
        ...
        & (data_selection["Datetime"] < date_to)
    ]
    ...
    truncated_array = np.maximum(
        data_matrix[:, 0], np.zeros(len(data_matrix[:, 0]))
    ).reshape(-1, 1)
    ...
    relative_power_vec = (
        data_matrix_truncated[:, 0] / data_matrix_truncated[:, 1] * 100
    ).reshape(-1, 1)
    ...
    seasonal_diff_y_vec = difference_vec_comp(
        y_vec=relative_power_vec, seasonality=int(4 * 24 * 365.25)
    )
